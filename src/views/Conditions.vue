  ...
  <div class="conditions container">
    ...
    <div class="prefilters">
      <div class="prefilter-el"
        ...
        @click="setList(el)"
        :class="{'selected': (el.v === currentPrefilter.v)}"
        :key="i">
        {{el.label}}
      </div>
    </div>
    ...
    <div class="assessments-list" v-if="showList">
      ...
      <div class="button" @click="currentSlice = currentSlice + 100" v-if="(currentSlice < currentList.length)">Load more...</div>
    </div>
...
export default {
  ...
  computed: {
    ...
    filteredAssessments() {
      return (this.activeFilters.length) ? this.customFilter(this.assessments, this.activeFilters) : this.assessments
    },
    randomAssessments() {
      return shuffle(this.filteredAssessments)
    },
    lowReviewed() {
      return this.filteredAssessments.filter((el) => el.no_assessments <= 4)
